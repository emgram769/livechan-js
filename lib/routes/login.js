  ...
  user_db.find({captcha: req.session.captcha}).exec(function(e,d) {
    if (d.length > 0) {
      ...
      if (secure_text === req.session.captcha) {
        ...
        user_db.find({ip: user_ip}, function(e, d){
          if(d[0] && d[0].ip) {
            // don't clear banned_rooms or ban_offense
            user_db.update({ip: user_ip}, {
              $set:{
                session_key: password,
                captcha: req.session.captcha
              }
              ...
              if (err) {
                ...
                console.log('existing user, banned? in', d.banned_rooms, 'offense', d.ban_offense);
              }
            })
          } else {
