...
module.exports = function(data, callback) {
    ...
    try {
        ...
        if (category === 'image') {
            var command = 'identify -format "{\\"width\\":%w,\\"height\\":%h}" '+clean_name+'[0]';
        } else if (category === 'video' || category === 'audio') {
            var command = 'ffprobe -print_format json -show_format -show_streams '+clean_name;
        } else {
        ...
        exec(command, function(err, stdout, stderr) {
            ...
            try {
                ...
                console.log('command returned unparseable metadata', command, stderr);
                return callback(new Error('metadata_error'));
            }

            if (category === 'image') {
                data.image_width = metadata.width;
                data.image_height = metadata.height;
                return callback();
            } else if (category === 'video' || category === 'audio') {
