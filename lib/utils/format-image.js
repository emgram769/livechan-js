...
module.exports = function(data, callback) {
    try {
        ...
        if (category === 'image') {
            command = 'identify';
            if (extension === 'gif') {
                args = '-format %T, - -format 0],["%m",%W,%H,"%A"]] -'.split(' ');
                args[2] = data.image;
                args[5] = data.image + '[0]';
            } else if (extension === 'png') {
                args = '-format 0,0],["%m",%w,%h,"%A"]] -'.split(' ');
                args[2] = data.image + '[0]';
            } else if (extension === 'jpg' || extension === 'jpeg') {
                args = '-format 0,0],["%m",%w,%h,"False"]] -'.split(' ');
                args[2] = data.image + '[0]';
            }
    ...
    try {
        ...
        process.on("close", function(code) {
            ...
            if (category === 'image') {
                if (!(metadata.length === 2 && metadata[0].length >= 2 && metadata[1].length === 4)) {
                    console.log('identify returned invalid metadata', stderr);
                ...
                data.image_transparent = (metadata[1][3].toLowerCase() === "true");
