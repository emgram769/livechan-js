...
module.exports = function(data, callback) {
    ...
    chat_db.update({count: data.count}, data, {upsert: true}, function(err) {
        ...
        callback();
        chat_db.find({chat: data.chat, convo: data.convo, is_convo_op: false})
            .sort({count: -1})
            .skip(150)
            .exec(delete_posts);
            
        chat_db.find({chat: data.chat, convo: "General"})
            .sort({count: -1})
            .skip(150)
            .exec(delete_posts);
        ...
        chat_db.aggregate([
					...
					{$group:{_id:"$convo",latest:{$max: "$count"}}},
					{$sort : { latest : -1 } },
					{$skip: 11}
				])
		.exec(function(err,candidates){
			if (candidates) {
				chat_db.find({
	        		chat: data.chat,
					convo: { $in: candidates.map(function(e){if (e._id != "General") return e._id;}) }
	        	}).exec(delete_posts);
        	}
		});
