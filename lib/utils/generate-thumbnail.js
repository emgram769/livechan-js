...
module.exports = function(data, callback) {
    ...
    try {
        ...
        if (category === "image") {
            var command = 'convert '+shell_escape(clean_name)+'[0] -layers coalesce -thumbnail '+thumb_width+'x'+thumb_height+'! '+(data.image_transparent?'+dither -colors 64 -strip ':'')+shell_escape(data.thumb);
        } else if (category === "video") {
            var command = 'ffmpeg -y -i '+shell_escape(clean_name)+' -s '+thumb_width+'x'+thumb_height+' -vframes 1 '+shell_escape(data.thumb);
        }
