...
module.exports = function(data, callback) {
    ...
    try {
        ...
        if (category === "image") {
            var command =
                'convert -[0] -layers coalesce -thumbnail '+thumb_width+'x'+thumb_height+'!'+
                (data.image_transparent ? ' +dither -colors 64 -strip png:-' : ' jpeg:-')+
                ' < '+shell_escape(clean_name)+' > '+shell_escape(data.thumb);
        } else if (category === "video") {
            var command =
                'ffmpeg -y -i - -s '+thumb_width+'x'+thumb_height+' -vframes 1 -f image2'+
