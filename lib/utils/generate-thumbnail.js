...
module.exports = function(data, callback) {
    ...
    try {
        ...
        data.thumb = path.join(__dirname, '..', '..', 'public/tmp/thumb', clean_base_names[1]+'.jpg');
        if (category === "image") {
            var command = "convert "+clean_name+"[0] -thumbnail "+thumb_width+"x"+thumb_height+"! "+data.thumb;
        } else if (category === "video") {
            var command = "ffmpeg -i "+clean_name+" -s "+thumb_width+"x"+thumb_height+" -vframes 1 "+data.thumb;
        }
