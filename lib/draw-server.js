...
fs.readFile(admin_pw_file, 'utf8', function(err, data) {
    if (!err) {
        ...
        config.admin_pw_salt = fields[0];
        config.admin_pw_hash = fields[1];
    }
});
		...
		switch (data[i].type) {
			...
			case "notification":
				...
				var hash = crypto.createHash('sha512')
							.update(config.admin_pw_salt, 'base64')
							.update(data[i].password, 'utf8').digest('base64');
				if (hash == config.admin_pw_hash) {
					console.log(data[i].message);
				} else {
					console.log("attempted pass", hash, data[i].password);
					can_send = false;
				}
				break;
			case "force_clear":
