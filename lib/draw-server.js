	...
	for (i in data) {
		...
		switch (data[i].type) {
			...
			case "image":
				...
				break;
			case "vote":
				if (!data[i].clear || (Object.keys(voted).indexOf(socket.handshake.address.address)>-1
					&& voted[socket.handshake.address.address] == data[i].clear)) {
...
io.sockets.on('connection', function (socket) {
  ...
  user_dict[socket.handshake.address.address] = [Date.now(), max_quota];
  ...
  socket.on('disconnect', function(){
	  delete user_dict[socket.handshake.address.address];
	  var user_count = Object.keys(user_dict).length;
	  io.sockets.emit('server', [{type:"user_count", count:user_count}]);
  })
});
